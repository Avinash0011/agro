...
function Register(){
    ...
    const confirmPasswordVisibility = CreatePassword('password_confirm'); // Use for confirm password field
...
// form submit in register
      const FormSubmit = async (e) =>{
          ...
          if(firstName=='' || phone=='' || password=='' || confirm==''){
            alert("Please fill all details");
            return false;
          }
          if(password!=confirm){
            alert("Password do not match, please fill same password");
            return false;
          }
        ...
        // try and catch method
            try {
              ...
              if (response.data.type ==true) {
                document.getElementById('category_form').reset(); // Replace with your form ID
              }
            } catch (error) {
...
<div className="col-xxl-7 col-xl-7 col-lg-12">
   <div className="row justify-content-center align-items-center h-100">
        <div className="col-xxl-8 col-xl-8 col-lg-7 col-md-7 col-sm-8 col-12">
            ...
            <form method="post"  onSubmit={FormSubmit} id='category_form'>
                <div className="row gy-3">
                    
                    <div className="col-xl-12">
                        <label htmlFor="signup-firstname" className="form-label text-default">First Name</label>
                        <input type="text" className="form-control form-control-lg" id="firstName" placeholder="Enter the first name" name="firstName"/>
                    </div>
                    ...
                    <div className="col-xl-12 mb-3">
                        <label htmlFor="password_confirm" className="form-label text-default">Confirm Password</label>
                        <div className="input-group">
                            <input type={confirmPasswordVisibility.isVisible?'text':'password'} className="form-control form-control-lg" id="password_confirm" placeholder="Confirm Password" name="password_confirm"/>
                            <button className="btn btn-light" onClick={confirmPasswordVisibility.toggleVisibility}
                            type="button" id="button-addon21"><i className={confirmPasswordVisibility.isVisible?'ri-eye-line align-middle':'ri-eye-off-line align-middle'}></i></button>
                        </div>
                    </div>
                    <div className="col-xl-12 d-grid mt-2">
                    {/* this alert div only when error and success*/}
                    { (errorMessage || successMessage ) && (<div className={`alert-dismissible alert alert-${errorMessage ? 'success': 'danger'}`} role="alert">
                        <i className="close submit-close ri-close-line" data-dismiss="alert" aria-label="close"></i>
                ...
                <div className="text-center">
                    <p className="fs-12 text-muted mt-4">Already have an account? <a href="login" className="text-primary">Sign In</a></p>
                </div>
            </div>
        </div>
    </div>
